...
// For each HTML box, there is an object in the DOM that we can access and ineract with
...
document.querySelector(".btn-roll").addEventListener("click", function() {
  if (gamePlaying) {
    ...
    if ((dice1 === 6 && lastDice === 6) || (dice2 === 6 && lastDice1 === 6)) {
      // Player loses score
...
function nextPlayer() {
  ...
  // Make dice image disappear when dice hits 1 and it's the next players turn
  hideDice();
}

document.querySelector(".btn-new").addEventListener("click", function() {
  var newGameConfirmation = window.confirm(
    "Are you sure you want to start a new game?"
  );
  newGameConfirmation;
  if (newGameConfirmation == true) {
    initializeGame();
  }
});
...
function initializeGame() {
  ...
  /* Will make the dice image disappear */
  hideDice();
